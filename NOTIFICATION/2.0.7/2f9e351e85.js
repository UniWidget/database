...
class Widget extends InvisibleWidget {
    constructor(props) {
        super(props);
          this.widgetLog('感谢使用Notification通知控件');
...
Widget.prototype.notification = function (title,data,) {
      notification = new Notification(title, JSON.parse(data));
}
types['methods'].push({
    key: 'options',
    label: 'options对象（改参数时请对照文档，不然可能无法正常显示）',
    params: [
      {
          key: 'dir',
          label: '文字方向（字符串）',

...
class Widget extends VisibleWidget {
    ...
    createMessage(type, position, name, avatar, msg, image, file, level, levelType, id, tag) {
        const result = { type, position, name, avatar, msg, image, file, level, levelType, tag }
    ...
    addMessage(data) {
        ...
        if (jsonData.type === 'charByChar' && jsonData.fullMsg) {
            this.startCharByCharAnimation(jsonData.id, jsonData.fullMsg.length)
        }
    }
        ...
        if (Array.isArray(parseData)) {
            ...
            parseData.forEach(item => {
                if (item.type === 'charByChar' && item.fullMsg) {
                    this.startCharByCharAnimation(item.id, item.fullMsg.length)
                }
            })
        } else {
    ...
    // 启动逐字输出动画
    startCharByCharAnimation(msgId, totalChars) {
        // 如果已经有定时器，先清除
    ...
    // 读取指定行的消息
    getLineMsg(line) {
        return JSON.stringify(JSON.parse(this.msgData)[line - 1])
    }
    ...
    avatarElement(data, index) { 
        const handleLongPress = (e) => {
            e.preventDefault()
            this.emit('onLongPressAvatar', index, data, e.clientX, e.clientY)
        }
        return (
    ...
    // 气泡 - 现在完全复用普通文本消息的渲染方式
    bubbleElement(data, index) { 
        const handleLongPress = (e) => {
            e.preventDefault()
            this.emit('onLongPressBubble', index, data, e.clientX, e.clientY)
        }
        
        // 逐字输出处理
        let displayMsg = data.msg
        if (data.type === 'charByChar' && data.fullMsg) {
            displayMsg = data.fullMsg.substring(0, data.charIndex || 0)
        }
        ...
        >
            <p style={{ fontSize: this.msgSize + 'px' }}>{displayMsg}</p>
        </div>
    )}
    ...
    messageElement(data, index) { return (
        <div className={`msg_item ${data.position}`} onClick={() => this.emit('onClickMessage', index, data)}>
            ...
            <div className={`content`}>
                ...
                { (data.type === 'text' || data.type === 'charByChar') && this.bubbleElement(data, index) }
                { data.type === 'richText' && this.bubbleElement(data, index) }
        ...
        <div className={`Qii_${this.__widgetId}`}>
            ...
            <style>
                {` 
                    ...
                    /* 最后一项添加动画 */
                    /* .Qii_${this.__widgetId} .msg_item:last-child {

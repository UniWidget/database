...
class Widget extends VisibleWidget {
		...
		return (
			React.createElement("div", {
				...
				dangerouslySetInnerHTML: {
					__html: ('<form id="bcxUpCodeMaoCommunityFile" action="https://api.bcmcreator.cn/img/UpCodeMaoCommunityFile.php" method="post" enctype="multipart/form-data"><input type="text" name="token" value="' + this.inputtext + '" hidden/><input type="button" id="filename"  value="' + this.tips + '" onclick="document.getElementById(\'bcxfileup\').click()" style="' + this.filestyle + '"><input type="file"   style="opacity: 0;" name="file"  id="bcxfileup" required="" onchange="document.getElementById(\'filename\').value=this.files[0].name;" />  ')
				}
			}, null)
		);
	...
	bcxgetform = () => {
		return $('#bcxUpCodeMaoCommunityFile input[name="file"]')
			.prop('files')
			.length ? false : true;
	};
	bcxsetform = () => {
		var file = $('#bcxUpCodeMaoCommunityFile input[name="file"]');
		file.after(file.clone()
			.val(""));
		file.remove();
		$('#bcxUpCodeMaoCommunityFile input[id="filename"]')
			.val(this.tips)

	};
set_token=(token)=>{
	 this.setProps({ 'inputtext': token });
	
}
	bcxform = () => {
		if(!this.inputtext) {
			this.widgetWarn('请先配置个人账户Cookie（点猫官方需校检身份）');
			return;
		}
		var targetUrl = $("#bcxUpCodeMaoCommunityFile")
			.attr("action");
		var data = new FormData($("#bcxUpCodeMaoCommunityFile")[0]);
		$.ajax({
			type: 'post',
			url: targetUrl,
			cache: false,

...
const types = {
    ...
    type: "MEDIA_WIDGET",
    icon: "",
    title: "摄像机",// Tips:ReducedRadius 修复
    version: "1.0.2",
...
class Widget extends VisibleWidget {
    ...
    run() {
        ...
        tool_getUserMedia({ video: { width: 480, height: 320, facingMode:'environment' } }, (stream) => {
    ...
    run_front() {
        var video = document.getElementById("MEDIA_" + this.__widgetId);
        // https://developer.mozilla.org/en-US/docs/Web/API/MediaTrackConstraints/facingMode        这里有相关讲解,不过是纯英文,前置是"user",后置为"environment"
        tool_getUserMedia({ video: { width: 480, height: 320, facingMode:'user' } }, (stream) => {
            var CompatibleURL = window.URL || window.webkitURL;
            try {
                video.src = CompatibleURL.createObjectURL(stream);
            } catch (e) {

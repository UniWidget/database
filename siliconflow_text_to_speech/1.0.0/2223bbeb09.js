...
class SpeechSynthesizer extends InvisibleWidget {
  ...
  async synthesizeSpeech(
    ...
    // 请求唯一标识 
    const requestId = `tts_${Date.now()}_${Math.random().toString(36).substr(2,  8)}`;
    
    // === 参数验证 === 
    const validateParam = (validator, errorMsg) => {
      if (!validator()) {
        this.emit("onSynthesisFailed",  400, errorMsg, requestId);
        return false;
      }
      return true;
    };
    ...
    // === 构建API请求 === 
    const apiUrl = `${baseApi}/audio/speech`;
    ...
    this.requestTimers[requestId]  = setInterval(() => {
      ...
      this.emit("onProgress",  progress, "正在合成语音");
    }, 1000);
 
    this.emit("onProgress",  0, "开始处理请求");
    ...
    try {
      ...
      if (responseFormat === 'mp3') {
        ...
        // 其他格式处理 
        const audioBlob = new Blob([response.data], { type: `audio/${responseFormat}` });
        audioUrl = URL.createObjectURL(audioBlob); 
      }
 
      this.emit("onProgress",  100, "合成完成");
      ...
      this.emit("onSpeechReady",  audioUrl, metadata);
      return { success: true, audioUrl, metadata };
    } catch (error) {
      clearInterval(this.requestTimers[requestId]); 
      delete this.requestTimers[requestId]; 
      ...
      this.emit("onSynthesisFailed",  errorCode, errorMessage, serverRequestId || requestId);

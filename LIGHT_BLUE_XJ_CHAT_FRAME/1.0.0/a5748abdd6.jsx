    ...
    mybeforeRenderingHTML = (e = '', empty = false) => {
        ...
        function beforeRenderingHTML(e, s) {
            ...
            for (let s = 0; s < e.length; s++) {
                !e[s].isRender &&
                    (-1 !== e[s].messageType.indexOf('tips')
                        ? (t += renderTipHtml(e[s].html, TipsType[e[s].messageType] || 'tips', s))
                        : (t += renderMessageHtml(e[s], s)));
            }
            n.innerHTML += t;
            if (that.autoRoll) {
                setTimeout(() => {
            ...
            for (const i of document.querySelectorAll('#lite-chatbox > div')) {
                if (i.getAttribute('class') === 'tips') {
                    ...
                    i.querySelector('span.content').addEventListener('mousedown', function (event) {
                        that.emit('onmousedown', Number(event.target.getAttribute('name')) + 1);
                        that.aaatimeStart = Date.now();
                        console.log(event);
                        that.aaatime = setInterval(() => {
                            if (Date.now() - that.aaatimeStart > 1000) {
                                clearInterval(that.aaatime);
                                that.emit(
                                    'onpressandhold',
                                    Number(event.target.getAttribute('name')) + 1,
                                    Number(event.x),
                                    Number(event.y)
                                );
                            }
                    ...
                    i.querySelector('span.content').addEventListener('mouseup', function (event) {
                        that.emit('onmouseup', Number(event.target.getAttribute('name')) + 1);
                        console.log(that.aaatime);
                        clearInterval(that.aaatime);
                    });
        ...
        function renderMessageHtml(e, s) {
            return `<div class="c${e.position} cmsg" name=${s}><img name=${s} class="headIcon ${
                e.diamond ? '' : 'radius'
            }"src="${
                e.headIcon
            }"ondragstart="return false;"oncontextmenu="return false;"/><span name=${s} class="name">${renderTitleHtml(
                e.htitle,
                TitleType[e.htitleType] || ''
            )}<span>${escapeHtml(e.name) || '&nbsp;'}</span></span><span name=${s} class="content" ${
                'colorDown' in e && 'colorUp' in e && 'color' in e
                    ? `style="background: linear-gradient(20deg, ${e.colorDown} 0, ${e.colorUp} 100%);color: ${e.color}"`
                    : ''
            }>${'raw' === e.messageType ? e.html : escapeHtml(e.html)}</span></div>`;
        }
        ...
        function renderTipHtml(e, s, realS) {
            return e
                ? `<div class="tips" name=${realS}><span name=${realS} class="${s}"style="margin-bottom: 20px;">${escapeHtml(
                      e
                  )}</span></div>`
                : '';
        }
        ...
        return React.createElement(
            'div',
            {
                dangerouslySetInnerHTML: {
                    __html: `<div id="lite-chatbox" class="lite-chatbox" style="overflow:auto;height:${this.realHeight}px;width:${this.realWidth}px"></div><style>
        html {
          line-height: 1.15;

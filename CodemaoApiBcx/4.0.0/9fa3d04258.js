...
types.docs = {
	url: "https://apifox.com/apidoc/shared-980cbe3c-ed2a-4c03-92cb-2b230794c025"
};
...
class Widget extends InvisibleWidget {
	...
	qrscan = (key, sign, time) => {
		...
		$.ajax("https://coco.codemao.cn/http-widget-proxy/https@SEP@api.pgaot.com/login/qr_login_sign", {
			method: "POST",
			...
			success: (result) => {
				this.emit('get_data', result)
			}
		}
		)
	};
	api = (lj, fs, type, cookie, data) => {
		$.ajax("https://coco.codemao.cn/http-widget-proxy/https@SEP@" + type + ".api.pgaot.com/" + lj, {
			method: fs,
			data: data,

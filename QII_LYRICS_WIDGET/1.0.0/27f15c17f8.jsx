...
class Widget extends VisibleWidget {
    ...
    setMusicTime(currentTime) {
        ...
        if (!this.isInit) {
            this.isInit = true
            lyricWidget.addEventListener('wheel', () => this.pauseScrolling())
            lyricWidget.addEventListener('touchstart', () => this.pauseScrolling())
            lyricWidget.addEventListener('touchmove', () => this.pauseScrolling())
        }
    }
    ...
    pauseScrolling() {
        ...
        // 标记正在滑动
        this.scrolling = true
        // 3 秒后重新设置延长器，如果 3 秒内被滑动，重新设置标记为 false。
    ...
    render() {
        return (
            <div className={`Qii_${this.__widgetId} ${this.lyricsString != '' ? 'show' : ''} ${this.lyricBlur ? 'blur' : ''}`}>

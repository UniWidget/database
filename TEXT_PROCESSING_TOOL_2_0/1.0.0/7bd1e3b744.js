// 控件类型定义
const types = {
    type: 'TEXT_PROCESSING_TOOL_2_0',
    icon: 'https://static.codemao.cn/pickduck/SJoMUWRege.svg?hash=Fol6gYnNnCwPdUj9Fk9aRSsxdkhe',
    title: '文字工具',
    docs: {
        url: 'https://www.yuque.com/xiaofeng-0fczj/rie0lc/gvptr5c48ne0hchy'
    },
...
class TextProcessingTool extends InvisibleWidget {
    ...
    getSubstringByRange(text, start, end) {
        if (typeof text !== 'string') throw new Error('内容不是字符串');
    ...
    getSubstring(text, length) {
        ...
        if (typeof length !== 'number' || length < 0) throw new Error('长度必须是正整数');
...
// 标明作者
console.log('作者：垃圾桶，二创作者：小风（QQ2085270395）');
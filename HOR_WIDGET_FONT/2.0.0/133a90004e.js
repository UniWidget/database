...
const defaultFontUrl = [{
    ...
    key:'MiSans VF',
    value:'https://static.codemao.cn/flowchunkflex/HJxgbmCZxe.ttf?hash=luagtlk3JrRe0JNnFwoKRDPJ8FAY'
},{
    key:'平方韶华体',
    value:'https://static.codemao.cn/flowchunkflex/S1U79dSZxx.ttf?hash=Fpf_r16p6BBs5XEq0Jsak0rcKT5l'
},{
    key:'阿里妈妈方圆体VF',
    value:'https://static.codemao.cn/flowchunkflex/HJ_CGX0Zxx.woff2?hash=Fh3w1gVL9DbTvgcCPowVTRMOpfn8'
},{
    key:'阿里妈妈灵动体VF',
    value:'https://static.codemao.cn/flowchunkflex/BkbfEXAZxl.woff2?hash=FqRZfC_f2HlRGVrWr1MpobRbPtE7'
}]

const types = {
    title: "字体库",
    type: "HOR_WIDGET_FONT",
    icon: "https://static.codemao.cn/flowchunkflex/r1zp0DBZee.svg?hash=FjE_hbQ4A0coI-pzglMQj67q63cC",
    docs: { url: 'https://www.yuque.com/seekyhor/horkj/ampt5x1s7al91zsl' },
    version: "2.0.0",
    ...
    methods: [
        ...
        },
        {
            ...
            blockOptions: { color: methodBlockColor, callMethodLabel: false, line: '控件字体',space: 40 },
        },
        {
            key: "letFontWidgetKeBian",
            label: '可变',
            tooltip: '设置控件的可变字体属性，比如字重、倾斜、圆角等，前提是控件的字体是可变字体；\n<设置>填轴名称和值，具体填写请见文档；<控件 ID>填要设置的控件的id。',
            params: [
                { key: "keBianValue", label: '设置', valueType: 'string', defaultValue: "'wght' 700, 'BEVL' 100" },
                { key: "widgetId", label: '控件 ID', valueType: 'string', defaultValue: 'WIDGET_ID' },
            ],
            blockOptions: { color: methodBlockColor, callMethodLabel: false },
        },
        {
            key: "letFontWidgetKeBianRt",
            label: '可变',
            valueType: 'string',
            tooltip: '返回值，填写到属性的<设置>中。\n<轴>的名称请根据所用字体的作者的设置填写。',
            params: [
                { key: "zhou", label: '轴', valueType: 'string', defaultValue: 'name' },
                { key: "zhi", label: '值', valueType: 'string', defaultValue: '100' },
            ],
            blockOptions: { color: createBlockColor, callMethodLabel: false },
        },
    ],
}
...
class Widget extends InvisibleWidget {
    ...
    letFontWidgetKeBian(keBianValue, widgetId) {
        if (!keBianValue || !widgetId) {
            this.widgetWarn('请正确填写设置<可变>字体的属性、控件 ID。');
            return;
        }
        ...
        // 直接设置 font-family（无论是否已存在）
        widget.style.fontVariationSettings = keBianValue;
    }


    letFontWidgetKeBianRt(zhou, zhi){
        return `'${zhou}' ${zhi}`;
    }
}

...
const createKey = (a,b)=>{var key = '';//乱码生成器
for (var i = 0; i < a; i++) {
var nKey = '';
for (var j = 0; j < b; j++) {
...
class Widget extends VisibleWidget {//控件外观/函数代码
    ...
    constructor(props) {
        super(props);
        this.url = props.url
        this.vkeyn = "XH_MUSIC_TOOL_WIDGET-VideoId-100c-6j-"+createKey(100,6)+(new Date().valueOf())
    ...
    setvolume(n){this.isinited()
       this.videoHtmlE.volume=n/100
    }
    novolume(){this.isinited()
       this.videoHtmlE.volume=0
    }
    setmultiple(x){this.isinited()
    ...
    initall(){
         ...
         document.getElementById(this.vkeyn).oncanplay=(e)=>{this.emit("oncanplay")}
         ...
         document.getElementById(this.vkeyn).onseeking=(e)=>{this.emit("onseeking")}

  ...
  methods: [
    {
      ...
      valueType: 'boolean',
      params: [
        { key: 'text', label: '检测输入文本（内容）中是否包含汉字，要检测的文本（内容）为：', valueType: 'string', defaultValue: '' },
      ],
      tooltip: '检测输入文本（内容）中是否包含汉字'
    },
    {
      ...
      tooltip: '返回输入文本（内容）中所有汉字字符的索引位置'
    },
    {
      key: 'getChineseCharacterCount',
      valueType: 'number',
      params: [
        { key: 'text', label: '返回输入文本（内容）中汉字的数量，输入文本（内容）为：', valueType: 'string', defaultValue: '' },
      ],
      tooltip: '返回输入文本（内容）中汉字的数量'
    },
    {
      key: 'isPureChinese',
      valueType: 'boolean',
      params: [
        { key: 'text', label: '检查输入文本（内容）是否完全由汉字组成，要检查的文本（内容）为：', valueType: 'string', defaultValue: '' },
      ],
      tooltip: '检查输入文本（内容）是否完全由汉字组成'
    },
    {
    ...
    },
    {
      ...
      tooltip: '统计输入文本（内容）中每个汉字字符的出现次数'
    },
    {
    ...
    },
    {
      ...
      tooltip: '在输入文本（内容）中找到最后一个汉字字符的索引位置'
    },
    {
    ...
    {
      key: 'removeNonChineseAndDigits',
      valueType: 'string',
      params: [
        { key: 'text', label: '从输入文本（内容）中删除所有非汉字和非数字的字符，要处理的文本（内容）为：', valueType: 'string', defaultValue: '' },
      ],
      tooltip: '从输入文本（内容）中删除所有非汉字和非数字的字符'
    },
    {
      key: 'getWidgetAuthor',
      valueType: 'string',
...
class ChineseDetectorWidget extends InvisibleWidget {
  ...
  removeNonChineseAndDigits = (text) => {
    return Array.from(text).filter(isChineseOrDigit).join('');
  };

  getWidgetAuthor = () => {
    return '控件版本号：2.1；共计13个功能；开发人员为kimi ai（主创），垃圾桶°（项目发起人，辅助创作，测试，监工）；版本更新日期：2025.2.4';
  }
}

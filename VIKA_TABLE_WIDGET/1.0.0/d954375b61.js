...
class Widget extends InvisibleWidget {
  constructor(props) {
    ...
    if (!this.token) {
      this.widgetError('初始化维格表失败，token 不能为空');
      return;
    }
    this.vika = new Vika({
      token: this.token,
  ...
  // 创建数据
  create() {
    console.info('vika create dict', this.rowDict);
    ...
    this.datasheet.records
      ...
      .then((data) => {
        ...
        this.widgetLog(`创建数据：${JSON.stringify(data)}`);
        if (
          data.success &&
          data.data &&
          data.data.records &&
